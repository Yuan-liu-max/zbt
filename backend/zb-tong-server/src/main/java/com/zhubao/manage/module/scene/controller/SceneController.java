...
public class SceneController {
    ...
    public ApiResult<PageResult<SceneHealthInspection>> pageHealth(@Valid PageDTO dto,
        ...
        LambdaQueryWrapper<SceneHealthInspection> w = new LambdaQueryWrapper<>();
        if (store != null) { w.eq(SceneHealthInspection::getStoreId, store); }
        if (StringUtils.isNotBlank(startDate)) { w.ge(SceneHealthInspection::getInspectionDate, LocalDate.parse(startDate)); }
        if (StringUtils.isNotBlank(endDate)) { w.le(SceneHealthInspection::getInspectionDate, LocalDate.parse(endDate)); }
        // 卫生巡检无 status 字段，不参与过滤
        IPage<SceneHealthInspection> r = svc.page(healthMapper, dto, w); return ApiResult.ok(PageResult.of(r)); }
    ...
    @OperateLog(module = "场景运营", action = "CREATE", targetType = "HEALTH_INSPECTION")
    @ApiOperation("新增卫生巡检") @PostMapping("/health-inspections")
    public ApiResult<Void> createHealth(@Valid @RequestBody SceneHealthInspection e) {
        if (e.getInspectionTime() == null || e.getInspectionTime().trim().isEmpty()) {
            e.setInspectionTime("09:00-18:00");
        }
        svc.save(healthMapper, e); return ApiResult.ok(); }
    @OperateLog(module = "场景运营", action = "UPDATE", targetType = "HEALTH_INSPECTION", targetIdExpr = "#id")
    ...
    @OperateLog(module = "场景运营", action = "DELETE", targetType = "HEALTH_INSPECTION", targetIdExpr = "#id")
    ...
    public ApiResult<PageResult<SceneDisplayInspection>> pageDisplay(@Valid PageDTO dto,
            ...
            @RequestParam(required = false) String startDate,
            @RequestParam(required = false) String endDate) {
        LambdaQueryWrapper<SceneDisplayInspection> w = new LambdaQueryWrapper<>();
        if (store != null) { w.eq(SceneDisplayInspection::getStoreId, store); }
        if (StringUtils.isNotBlank(startDate)) { w.ge(SceneDisplayInspection::getInspectionDate, LocalDate.parse(startDate)); }
        if (StringUtils.isNotBlank(endDate)) { w.le(SceneDisplayInspection::getInspectionDate, LocalDate.parse(endDate)); }
        // 陈列检查无 status 字段，不参与过滤
        IPage<SceneDisplayInspection> r = svc.page(displayMapper, dto, w); return ApiResult.ok(PageResult.of(r)); }
    ...
    @OperateLog(module = "场景运营", action = "CREATE", targetType = "DISPLAY_INSPECTION")
    ...
    @OperateLog(module = "场景运营", action = "UPDATE", targetType = "DISPLAY_INSPECTION", targetIdExpr = "#id")
    ...
    @OperateLog(module = "场景运营", action = "DELETE", targetType = "DISPLAY_INSPECTION", targetIdExpr = "#id")
    ...
    public ApiResult<PageResult<SceneMaterialUpdate>> pageMaterial(@Valid PageDTO dto,
        ...
        LambdaQueryWrapper<SceneMaterialUpdate> w = new LambdaQueryWrapper<>();
        if (store != null) { w.eq(SceneMaterialUpdate::getStoreId, store); }
        if (StringUtils.isNotBlank(startDate)) { w.ge(SceneMaterialUpdate::getCheckDate, LocalDate.parse(startDate)); }
        if (StringUtils.isNotBlank(endDate)) { w.le(SceneMaterialUpdate::getCheckDate, LocalDate.parse(endDate)); }
        if (StringUtils.isNotBlank(currentStatus)) { w.eq(SceneMaterialUpdate::getCurrentStatus, currentStatus); }
        IPage<SceneMaterialUpdate> r = svc.page(materialMapper, dto, w); return ApiResult.ok(PageResult.of(r)); }
    ...
    @OperateLog(module = "场景运营", action = "CREATE", targetType = "MATERIAL_UPDATE")
    ...
    @OperateLog(module = "场景运营", action = "UPDATE", targetType = "MATERIAL_UPDATE", targetIdExpr = "#id")
    ...
    @OperateLog(module = "场景运营", action = "DELETE", targetType = "MATERIAL_UPDATE", targetIdExpr = "#id")
    ...
    public ApiResult<PageResult<SceneEquipmentCheck>> pageEquipment(@Valid PageDTO dto,
        ...
        LambdaQueryWrapper<SceneEquipmentCheck> w = new LambdaQueryWrapper<>();
        if (store != null) { w.eq(SceneEquipmentCheck::getStoreId, store); }
        if (StringUtils.isNotBlank(startDate)) { w.ge(SceneEquipmentCheck::getCheckDate, LocalDate.parse(startDate)); }
        if (StringUtils.isNotBlank(endDate)) { w.le(SceneEquipmentCheck::getCheckDate, LocalDate.parse(endDate)); }
        if (StringUtils.isNotBlank(status)) { w.eq(SceneEquipmentCheck::getStatus, status); }
        IPage<SceneEquipmentCheck> r = svc.page(equipmentMapper, dto, w); return ApiResult.ok(PageResult.of(r)); }
    ...
    @OperateLog(module = "场景运营", action = "CREATE", targetType = "EQUIPMENT_CHECK")
    ...
    @OperateLog(module = "场景运营", action = "UPDATE", targetType = "EQUIPMENT_CHECK", targetIdExpr = "#id")
    ...
    @OperateLog(module = "场景运营", action = "DELETE", targetType = "EQUIPMENT_CHECK", targetIdExpr = "#id")
    ...
    @OperateLog(module = "场景运营", action = "CREATE", targetType = "EXPERIENCE_REVIEW")
    ...
    @OperateLog(module = "场景运营", action = "UPDATE", targetType = "EXPERIENCE_REVIEW", targetIdExpr = "#id")
    ...
    @OperateLog(module = "场景运营", action = "DELETE", targetType = "EXPERIENCE_REVIEW", targetIdExpr = "#id")
