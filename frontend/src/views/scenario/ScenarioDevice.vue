  ...
  <div class="page-container">
    ...
    <div class="content-card table-card">
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <!-- 维修 -->
          <template v-if="column.key === 'repairRequired'">
            {{ record.repairRequired === 1 ? '是' : '否' }}
          </template>
          <!-- 操作 -->
    ...
    >
      <a-form ref="formRef" :model="formData" :rules="formRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        ...
        <a-form-item label="设备类型" name="equipmentType">
          <a-input v-model:value="formData.equipmentType" placeholder="如：收银机 / 空调 / 安防监控" />
        </a-form-item>
        <a-form-item label="状态" name="status">
          <a-input v-model:value="formData.status" placeholder="如：正常 / 异常 / 待维修" />
        </a-form-item>
        ...
        <a-form-item label="是否需要维修" name="repairRequired">
          <a-radio-group v-model:value="formData.repairRequired">
    ...
    <a-modal v-model:open="detailVisible" title="详情" :footer="null" width="600px">
      <a-descriptions :column="2" bordered size="small">
        ...
        <a-descriptions-item label="设备类型">{{ detailRecord?.equipmentType ?? '-' }}</a-descriptions-item>
        ...
        <a-descriptions-item label="是否需要维修">{{ detailRecord?.repairRequired === 1 ? '是' : '否' }}</a-descriptions-item>
...
import type { DeviceItem } from '@/types/scenario'
import { deviceApi } from '@/api/scene'
...
const columns = computed(() => [
  ...
  { title: '维修', key: 'repairRequired', width: 70, align: 'center' as const },
...
const detailRecord = ref<DeviceItem | null>(null)
...
const syncPhotoList = () => {
  photoUrlList.value = photoUrlsArray(formData.photoUrls).map((url, i) => ({ uid: `photo-${i}`, url }))
}
...
const handleEdit = (record: DeviceItem) => {
  ...
  formData.equipmentType = record.equipmentType || ''
  formData.status = record.status || ''
  formData.issueDescription = record.issueDescription || ''
  formData.repairRequired = record.repairRequired === 1 ? 1 : 0
...
const handleModalOk = async () => {
  try {
    ...
    const submitData = {
      ...
      repairRequired: formData.repairRequired,
      photoUrls: formData.photoUrls
    }
    if (isEdit.value) {
      await deviceApi.update(formData.id, submitData)
